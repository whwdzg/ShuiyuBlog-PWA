...
Write-Host "Installing 水鱼Blog..."
$currentDir = Get-Location
$msixPath = ($currentDir.Path + "\" + "水鱼Blog.sideload.msix")
...
if ($installProc.ExitCode -eq 0) {
	$app = Get-StartApps "水鱼Blog"

	# If it's an array, then we found multiple matching apps. Grab the last one.
	if ($app -is [array]) {
		Write-Host "Warning: found multiple apps installed named 水鱼Blog. Launching best guess. If the wrong app launches, find the right one in your start menu."
		$app = $app[-1];
	}

	if ($app) {
		Write-Host "Launching 水鱼Blog..."
